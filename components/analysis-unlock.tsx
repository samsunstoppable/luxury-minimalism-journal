...
export function AnalysisUnlock({ onSelectPersona, onCancel }: AnalysisUnlockProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center justify-between h-14 md:h-20">
            <button
              onClick={onCancel}
              className="font-sans text-[10px] md:text-xs tracking-widest uppercase text-muted-foreground hover:text-foreground transition-colors duration-300"
            >
              Back
            </button>
      ...
      {/* Main content */}
      <main className="flex-1 pt-20 md:pt-32 pb-32">
        <div className="max-w-6xl mx-auto px-6">
          {/* Headline */}
          <div className="text-center mb-10 md:mb-16 animate-in fade-in duration-700">
            <h1 className="font-serif text-2xl md:text-5xl text-foreground tracking-tight mb-3 md:mb-4">Choose Your Guide.</h1>
            <p className="font-sans text-xs md:text-base text-muted-foreground max-w-md mx-auto leading-relaxed">
              Select a perspective through which to explore your seven days of reflection.
            </p>
          </div>

          {/* Persona Grid - 2x5 on mobile, responsive on larger screens */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150">
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-background/80 backdrop-blur-sm border-t border-border/50">
        <div className="max-w-6xl mx-auto px-6 py-4 md:py-6">
          <div className="flex items-center justify-between">
            {/* Selection status */}
            <span className="font-sans text-[10px] md:text-xs text-muted-foreground tracking-wider">
              {selectedPersona ? (
                <span>
            ...
            <button
              ...
              className="font-sans text-[10px] md:text-xs tracking-widest uppercase text-foreground border-b border-foreground pb-0.5 hover:opacity-60 transition-all duration-300 disabled:opacity-30 disabled:border-muted-foreground disabled:cursor-not-allowed"
            >
              Begin Analysis
            </button>
          </div>
